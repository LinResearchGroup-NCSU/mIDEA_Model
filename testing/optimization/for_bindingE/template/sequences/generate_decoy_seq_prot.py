...
sys.path.append('/Users/xl23/Working/future/protein_DNA_interaction_optimization/code/common_functions')
    ...
    for i, method in enumerate(methods):
        ...
        for protein in protein_list:
            ...
            for j in range(num_decoys[i]):
                newsequence = generate_decoy_sequence(protein, method=method, degree=j) 
                while (newsequence == None):
                    # If returning none, it means the sampled sequence happen to be within the good binding sequences;
                    # Then we redo the generation process until it returns a new sequence
                    newsequence = generate_decoy_sequence(protein, method=method, degree=j) 
                    
                output_file.write(newsequence + '\n')
            output_file.close()
        os.chdir('..')
...
def generate_decoy_sequence(protein, method='DNA_randomization', degree=None):
    ...
    if method == 'shuffle':
        ...
        resids_toberandomized = open(
            "randomize_position_DNA.txt", 'r').readline().strip().split(' ')
...
generate_decoy_sequences("proteins_tmp_list.txt", methods=['prot_randomization'], num_decoys=[1000], randomSeed=0)
