...
def get_neighbors_within_radius_native(neighbor_list, residue, radius):
    #print(residue.resname)
    return neighbor_list.search(get_interaction_atom_native(residue).get_coord(), radius, level='R')
def get_neighbors_within_radius_decoy(neighbor_list, residue, radius):
    #print(residue.resname)
    return neighbor_list.search(get_interaction_atom_decoy(residue).get_coord(), radius, level='R')


def get_interaction_atom_native(residue):
    nucleotide_bases = ["DA", "DT", "DC", "DG"]
    residue_name = residue.resname.strip()
    if residue_name in nucleotide_bases:
#        atom_order = ["C1'"]
#        atom_order = ["C5"]
        atom_order = ["P", "O5'"]
    else:
#        atom_order = ["CH", "CZ", "CE", "CD", "CG", "CB", "CA"]
#        atom_order = ["CB", "CA"]
        atom_order = ["CA"]
    for atom_name in atom_order:
        if atom_name in residue:
            return residue[atom_name]
    raise ValueError(f"No suitable interaction atom found for residue: {residue_name}")    

def get_interaction_atom_decoy(residue):
    nucleotide_bases = ["DA", "DT", "DC", "DG"]
    residue_name = residue.resname.strip()
    if residue_name in nucleotide_bases:
#        atom_order = ["C1'"]
#        atom_order = ["C5"]
        ...
        # Special for the 1hlo_Maerki dataset;
#        atom_order = ["CH", "CZ", "CE", "CD", "CG", "CB", "CA"]
#        atom_order = ["CB", "CA"]
        atom_order = ["CA"]
    for atom_name in atom_order:
...
def get_interaction_distance_native(res1, res2):
    return get_interaction_atom_native(res1) - get_interaction_atom_native(res2)

def get_interaction_distance_decoy(res1, res2):
    return get_interaction_atom_decoy(res1) - get_interaction_atom_decoy(res2)
